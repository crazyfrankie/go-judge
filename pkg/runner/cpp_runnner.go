...
func (c *CppRunner) RunWithSandbox(ctx context.Context, req *rpc.JudgeRequest) (*rpc.JudgeResponse, error) {
	// build work dir
...
func (c *CppRunner) RunWithDocker(ctx context.Context, req *rpc.JudgeRequest) (*rpc.JudgeResponse, error) {
	return nil, nil
}

func (c *CppRunner) runWithNativeSandbox(limit *types.Limit, inputs []string, expectedOutputs []string, workdir string) ([]*types.TestCaseResult, error) {
...
func (c *CppRunner) parseCTemplate(fullTemp string, userCode string, types string, workdir string) error {
	tpl, _ := template.New("main").Delims("[[", "]]").Parse(fullTemp)
	codeFile, _ := os.OpenFile(fmt.Sprintf("%s/main.cpp", workdir), os.O_CREATE|os.O_RDWR, 0644)
...
func (c *CppRunner) preCompile(workdir string) (*types.PreCompileResult, error) {
	...
	defer sh.Close()
	if _, err = sh.WriteString(fmt.Sprintf(types.CppBuildShell, workdir)); err != nil {
		return nil, err
	}
